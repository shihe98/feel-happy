...
**零正样本免疫异常行为的漂移，但正常性发生变化需要考虑，以无监督的方式检测变化，减少手动标注的开销**
...
**基于边界自适应随机平滑的DL基础流量分析系统的通用强健性认证框架**
...
**自动将后门样本分解为干净的输入和相应的触发器，然后分类并总结各种攻击模式，最后查询其他模型是否存在相同的后门【网络攻击与后门攻击关联，做自动化工具】**
...
**【数据集指纹或者后门攻击】大型数据集的知识压缩为一个更小的合成数据集，在精炼前或精炼中添加触发器**

7. RoVISQ: Reduction of Video Service Quality via Adversarial Attacks on Deep Learning-based Video Compression<br>
...
**视频压缩和下游分类系统的对抗攻击（1）增加网络带宽或降低终端用户的视频质量（2）下游视频分类服务的有针对性和无针对性攻击【压缩和后门很配】**
8. The Beatrix Resurrections: Robust Backdoor Detection via Gram Matrices<br>
...
**Gram矩阵来捕捉特征之间的相关性，区分正常样本与异常样本，利用假设检验继续提高精度**
## 人工智能&隐私保护
