...
**分散学习增加了攻击面，任何用户都能够执行隐私攻击，获得对诚实用户的本地模型的完全控制**
...
**联邦学习训练方法，DPSGD算法与两种优化方法(数据无关特征提取&基于本地数据的全局模型初始化方法)**

9. SNAP: Efficient Extraction of Private Properties with Poisoning<br>
...
**财产推论攻击，污染一部分训练数据集并查询训练目标模型的财产推论攻击的设置**
10. Spectral-DP: Differentially Private Deep Learning through Spectral Perturbation and Filtering<br>
...
**Spectral-DP谱域梯度扰动与谱滤波相结合，以更低的噪声尺度和更好的效用实现所需的隐私保证**
## 人工智能&安全威胁
...
**针对3D人脸识别系统的新的攻击方式，使用一张2D照片欺骗3D人脸识别**
...
**后门可以用于深度学习的相关分析，欺骗模型预测与可解释性分析【考虑对深度学习的元操作】**
...
**生成风格变化的图片，可以化妆得到，导致攻击者可以冒充内部人员**
...
**重新考虑针对恶意软件分类器的后门攻击的隐蔽性，良性软件字节码做触发器，类似复合后门**
...
**克服了轻量级、提供防伪保护和具有平滑外观的指纹样本的公共验证挑战**
...
**连续学习中长期存在的灾难性遗忘（CF）问题【随机标记的干净数据上重新训练给定的DNN模型，以诱导CF】**
...
**综合研究对任何攻击提供鲁棒性准确性的鲁棒性验证和相应的鲁棒性训练方法**
...
**通过风格转移来欺骗视频分类系统的黑盒视频对抗性攻击，选风格，朝着目标扰动，最后无梯度优化【攻击效果，查询次数、鲁棒性】**
...
**针对自定语音识别后门攻击，旨在控制语音转文字任务，选择音乐为触发器**
